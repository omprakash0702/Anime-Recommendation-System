...
def find_similar_animes(name, path_anime_weights, path_anime2anime_encoded, path_anime2anime_decoded, path_anime_df, n=10, return_dist=False, neg=False):
    try:
        ...
        # Ensure proper shapes before np.dot
        target_vector = weights[encoded_index]
        if target_vector.ndim > 1:
            target_vector = np.squeeze(target_vector)
        ...
        print("Error Occured in find_similar_animes:", e)
        return pd.DataFrame()
...
def find_similar_users(item_input, path_user_weights, path_user2user_encoded, path_user2user_decoded, n=10, return_dist=False, neg=False):
    try:
        ...
        if encoded_index is None:
            print(f"[Warning] Encoded index not found for user_id: {index}")
            return pd.DataFrame(columns=["similar_users", "similarity"])

        weights = user_weights

        # Ensure vector shape consistency before np.dot
        target_vector = weights[encoded_index]
        if target_vector.ndim > 1:
            target_vector = np.squeeze(target_vector)
        ...
        for close in closest:
            similarity = float(dists[close])  # ensure numeric type
            decoded_id = user2user_decoded.get(close, None)
        ...
        # Safety: ensure the columns exist even if empty
        if "similar_users" not in similar_users.columns:
            similar_users["similar_users"] = []
        if "similarity" not in similar_users.columns:
            similar_users["similarity"] = []

        return similar_users.sort_values(by="similarity", ascending=False)

    except Exception as e:
        print("Error Occured in find_similar_users:", e)
        return pd.DataFrame(columns=["similar_users", "similarity"])
...
def get_user_preferences(user_id, path_rating_df, path_anime_df):
    ...
    # Filter for the given user's ratings
    animes_watched_by_user = rating_df[rating_df.user_id == user_id]

    # 🧩 Check if the user exists or has any ratings
    if animes_watched_by_user.empty:
        print(f"[Warning] No ratings found for user_id: {user_id}")
        return pd.DataFrame(columns=["eng_version", "Genres"])

    # Safely calculate the 75th percentile rating
    user_rating_percentile = np.percentile(animes_watched_by_user.rating, 75)

    # Filter top-rated animes for this user
    ...
    # 🧠 Return even if empty, so downstream functions don’t break
    return anime_df_rows.reset_index(drop=True)
